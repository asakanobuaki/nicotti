class User < ApplicationRecord
  ...
  def today_smokings_count
    smokings.where(created_at: Time.zone.today.all_day).count
  end

  def yesterday_smokings_count
    smokings.where(created_at: Time.zone.yesterday.all_day).count
  end

  def monthly_smokings_count
    smokings.where(created_at: Time.zone.yesterday.all_month).count
  end

  # 1日の目標喫煙本数を超過している場合、超過本数が１プラスされる。
  def target_over
    return self.excess_cigarette += 1 if today_smokings_count > target_number
  end
  ...
  def set_body
    {
      ...
      contents: [
        {
          type: 'text',
          text: I18n.l(Time.zone.today, format: :long).to_s
        },
        {
          type: 'text',
          ...
          contents: [
            {
              ...
              contents: [
                {
                  type: 'text',
                  text: "#{I18n.l Time.zone.yesterday}の喫煙本数",
                  weight: 'bold',
                  margin: 'xs',
                  flex: 0,
                  size: 'md'
            ...
            },
            {
              ...
              contents: [
                {
                  type: 'text',
                  text: "#{Time.zone.yesterday.month}月の総喫煙本数",
                  weight: 'bold',
                  margin: 'sm',
                  flex: 0
                },
